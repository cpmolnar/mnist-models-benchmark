...
class inception_v2:
    ...
    def inception_block_factorized(self, val, num_1x1, num_3x3_reduce, num_3x3, num_5x5_reduce, num_5x5, pool_proj):
        branch_0 = self.conv_layer(val, num_outputs=num_1x1, kernel_size=1, stride=1, padding='SAME')
        
        branch_1 = self.conv_layer(val, num_outputs=num_3x3_reduce, kernel_size=1, stride=1, padding='SAME')
        branch_1 = self.conv_layer(branch_1, num_outputs=num_3x3, kernel_size=[1, 3], stride=1, padding='SAME')
        branch_1 = self.conv_layer(branch_1, num_outputs=num_3x3, kernel_size=[3, 1], stride=1, padding='SAME')

        branch_2 = self.conv_layer(val, num_outputs=num_5x5_reduce, kernel_size=1, stride=1, padding='SAME')
        branch_2 = self.conv_layer(branch_2, num_outputs=num_5x5, kernel_size=[1, 3], stride=1, padding='SAME')
        branch_2 = self.conv_layer(branch_2, num_outputs=num_5x5, kernel_size=[3, 1], stride=1, padding='SAME')
        branch_2 = self.conv_layer(branch_2, num_outputs=num_5x5, kernel_size=[1, 3], stride=1, padding='SAME')
        branch_2 = self.conv_layer(branch_2, num_outputs=num_5x5, kernel_size=[3, 1], stride=1, padding='SAME')
...
def model(X_train, X_val, y_train, y_val, print_cost = True, learning_rate = 0.0002, minibatch_size = 64, num_epochs = 1):
